...
class MTrep:
    def __init__(self, N=100, L=1, K=2, undirected=False, initialization=0, rseed=0, inf=1e10, err_max=1e-8, err=0.01,
                 N_real=1, tolerance=0.0001, decision=10, max_iter=500, out_inference=False, in_folder='../data/input/',
        ...
        if initialization not in {0, 1}:
            raise ValueError('The initialization parameter can be either 0 or 1. It is used as an indicator to '
                             'initialize the membership matrices U and V and the affinity tensor W. If it is 0, they '
    ...
    def fit(self, data, nodes, mask=None):
        ...
        for r in range(self.N_real):
            ...
            if self.verbose:
                print('done!')
                print(f'r = {r} - Loglikelihood = {loglik} - iterations = {it} - '
    ...
    def _initialize_u(self, infile_name):
        ...
        with open(infile_name, 'rb') as f:
            dfU = pd.read_csv(f, header=None, sep='\s+')
            self.u = dfU.values[:, 1:]
    ...
    def _initialize_v(self, infile_name):
        ...
        if self.undirected:
            ...
            with open(infile_name, 'rb') as f:
                dfV = pd.read_csv(f, header=None, sep='\s+')
                self.v = dfV.values[:, 1:]
    ...
    def _initialize_w(self, infile_name):
        ...
        with open(infile_name, 'rb') as f:
            dfW = pd.read_csv(f, header=None, sep='\s+', skiprows=1)
    ...
    def _update_cache(self, data, data_T_vals, subs_nz):
        """
            ...
            data : sptensor/dtensor
                   Graph adjacency tensor.
            data_T_vals : ndarray
                          Array with non zero entries of data_T.
            subs_nz : tuple
